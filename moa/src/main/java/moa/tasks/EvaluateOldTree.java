 ...
 * @version $Revision: 7 $
 */
public class EvaluateOldTree extends ClassificationMainTask {
    
	private Long firstDetectedChange;
	...
	*/
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
    	...
    	//copy stream and learner
    	//ConceptDriftStream testStream = (ConceptDriftStream) stream.copy(); 	
    	//SingleClassifierDrift oldTree = (SingleClassifierDrift) learner.copy();
    	SingleClassifierDrift oldTree = new SingleClassifierDrift();
        ...
        while (stream.hasMoreInstances() 
            ...
            oldTree = (SingleClassifierDrift) learner.copy();
    		learner.trainOnInstance(trainInst);
        	instancesProcessed++;
        	this.cique.offer(trainInst);
            //if change is detected, set the flag to true, otherwise train the model
        	if(learner.getChangeDetector().getChange()
        		&& instancesProcessed >= trueChangePoint){   		
        		this.firstDetectedChange = instancesProcessed;   
        		//System.out.println("first detected change: " + this.firstDetectedChange);
        ...
        //System.out.println("old tree:" + oldTree.toString());
    	...
    	while (iter.hasNext() 
            ...
            double[] prediction = oldTree.getVotesForInstance(testInst);
            // Output prediction
            if (outputPredictionFile != null) {
        	...
        	if (numberInstances % this.sampleFrequencyOption.getValue() == 0
                    ...
                    //|| testStream.hasMoreInstances() == false) {
                long evaluateTime = TimingUtils.getNanoCPUTimeOfCurrentThread();
                double time = TimingUtils.nanoTimeToSeconds(evaluateTime - evaluateStartTime);
                double timeIncrement = TimingUtils.nanoTimeToSeconds(evaluateTime - lastEvaluateStartTime);
                double RAMHoursIncrement = oldTree.measureByteSize() / (1024.0 * 1024.0 * 1024.0); //GBs
